...
void Agent::train(Env env,long epochs,long initialBias){
    ...
    for(int epoch=0;epoch<epochs;epoch++){
        ...
        if(this->epsilon>this->minEpsilon){
            this->epsilon = max(0.1, 1 - (epoch * 0.00005));
        ...
        if (epoch % 4000 == 0) { 
            // Compute average returns over the last 100 episodes
            int size = stats["Returns"].size();
            int start_idx = std::max(0, size - 100);
            double sum = std::accumulate(stats["Returns"].begin() + start_idx, stats["Returns"].end(), 0.0);
            double avg_returns = sum / std::min(100, size);

            stats["AvgReturns"].push_back(avg_returns);
            stats["EpsilonCheckpoint"].push_back(this->epsilon);
