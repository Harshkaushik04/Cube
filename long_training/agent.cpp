...
void Agent::train(Env env,long epochs,long initialBias){
    ...
    stats["veryHighRewards"].push_back(1);
        ...
        if(this->epsilon>this->minEpsilon){
            this->epsilon = max(0.1, 0.1 - (epoch * 0.0005));

        }
        if(env.stage!=Stage::Cross && this->epsilonStage2>this->minEpsilon){
            this->epsilonStage2=max(0.1,1-(epoch * 0.0005));
        }
    ...
    stats["veryHighRewardsAverage"].push_back(std::accumulate(stats["veryHighRewards"].begin() + std::max(0, size - 100), stats["veryHighRewards"].end(), 0.0));
        ...
        if (epoch % 1000 == 0) { 
            ...
            for (size_t i = 0; i < stats["AvgReturns"].size(); ++i) {
                file << stats["AvgReturns"][i] << "," << stats["EpsilonCheckpoint"][i] << stats["EpsilonStage2"][i]<<stats["lowRewardsAverage"]
                <<stats["mediumRewardsAverage"]<<stats["highRewardsAverage"]<<stats["veryHighRewardsAverage"]<<"\n";
            }
            file.close();
