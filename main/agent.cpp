...
// void ReplayBuffer::insert(std::tuple<torch::Tensor,torch::Tensor,torch::Tensor,torch::Tensor,torch::Tensor> transition){
//         std::tuple<torch::Tensor,torch::Tensor,torch::Tensor,torch::Tensor,torch::Tensor> cpuTuple = make_tuple(
...
Agent::Agent(CubeNetImpl model,torch::DeviceType device,torch::DeviceType largerMemoryDevice,float epsilon,float minEpsilon,
    int numWarmup,int numActions,long capacity,int batchSize,float learningRate,float gamma){
        shared_ptr<CubeNetImpl> modelPtr=make_shared<CubeNetImpl>(model);
        this->model=modelPtr;
        this->targetModel=modelPtr;
...
torch::Tensor Agent::getAction(torch::Tensor state){
...
void Agent::train(Env env,long epochs,long initialBias){
    ...
    for(int epoch=0;epoch<epochs;epoch++){
        ...
        if(this->epsilon>this->minEpsilon){
            this->epsilonDecay=0.08+0.92*exp(-0.00011*epoch);
...
void Agent::test(Env env){
    ...
    for(int epoch=0;epoch<10;epoch++){
         ...
         while(!done){
            action=this->getAction(state).to(device);
            result=env.step(action);
            next_state=get<0>(result);
            reward=get<1>(result);
            done=get<2>(result).item<bool>();
